...
var LNReaderPlugin = (() => {
  ...
  var ChireadsPlugin = (
    ...
    function() {
        ...
        this.filters = {
          tag: {
            ...
            options: [
              ...
              { label: "De faible \xE0 fort", value: "de-faible-a-fort" },
              { label: "Adapt\xE9 en manhua", value: "adapte-en-manhua" },
              ...
              { label: "Adapt\xE9 en Anim\xE9", value: "adapte-en-anime" },
              { label: "Alchimie", value: "alchimie" },
              { label: "\xC9l\xE9ments de jeux", value: "elements-de-jeux" },
              ...
              { label: "Syst\xE8me de niveau", value: "systeme-de-niveau" },
              { label: "Cheat", value: "cheat" },
              { label: "Protagoniste g\xE9nie", value: "protagoniste-genie" },
              { label: "Com\xE9die", value: "comedie" },
              { label: "Gamer", value: "gamer" },
              { label: "Mariage", value: "mariage" },
              { label: "seeking Protag", value: "seeking-protag" },
              { label: "Romance pr\xE9coce", value: "romance-precoce" },
              { label: "Croissance acc\xE9l\xE9r\xE9e", value: "croissance-acceleree" },
              { label: "Artefacts", value: "artefacts" },
              {
                label: "Intelligence artificielle",
                value: "intelligence-artificielle"
              },
              { label: "Mariage arrang\xE9", value: "mariage-arrange" },
              { label: "Mature", value: "mature" },
              { label: "Adulte", value: "adulte" },
              {
                label: "Administrateur de syst\xE8me",
              ...
              { label: "D\xE9mons", value: "demons" },
              { label: "Reincarnation", value: "reincarnation" },
              { label: "Acad\xE9mie", value: "academie" },
              {
                label: "Cacher les vraies capacit\xE9s",
              ...
              {
                label: "Protagoniste fort d\xE8s le d\xE9part",
              ...
              { label: "De pauvre \xE0 riche", value: "de-pauvre-a-riche" },
              {
                label: "Syst\xE8me de classement de jeux",
                value: "systeme-de-classement-de-jeux"
              },
              { label: "Capacit\xE9s sp\xE9ciales", value: "capacites-speciales" },
              { label: "Vengeance", value: "vengeance" }
            ]
          }
        };
      }
      __name(ChireadsPlugin2, "ChireadsPlugin");
      ChireadsPlugin2.prototype.getCheerio = function(url) {
      ...
      ChireadsPlugin2.prototype.popularNovels = function(pageNo_1, _a) {
        return __awaiter(this, arguments, void 0, function(pageNo, _b) {
          var url, tag, $2, novels, novel, loop, i, romans, populaire, novelCover_1, novelName_1, novelUrl_1, imgs_1, txts;
          var _this = this;
          ...
          return __generator(this, function(_c) {
            switch (_c.label) {
              case 0:
                ...
                return [4, this.getCheerio(url)];
              case 1:
                $2 = _c.sent();
                novels = [];
                if (!(showLatestNovels || tag !== "all")) return [3, 7];
                loop = 1;
                if (showLatestNovels)
                  loop = 2;
                i = 0;
                _c.label = 2;
              case 2:
                if (!(i < loop)) return [3, 6];
                if (!(i === 1)) return [3, 4];
                return [4, this.getCheerio(this.site + "/category/original/page/" + pageNo)];
              case 3:
                $2 = _c.sent();
                _c.label = 4;
              case 4:
                romans = $2(".romans-content li");
                if (!romans.length)
                  romans = $2("#content li");
                romans.each(function(i2, elem) {
                  var novelName = $2(elem).contents().find("div").first().text().trim();
                  var novelCover = $2(elem).find("div").first().find("img").attr("src");
                  var novelUrl = $2(elem).find("div").first().find("a").attr("href");
                  if (novelUrl) {
                    novel = {
                      name: novelName,
                      cover: novelCover,
              ...
              case 6:
                return [3, 8];
              case 7:
                populaire = $2(':contains("Populaire")').last().parent().next().find("li > div");
                if (populaire.length === 12) {
                  populaire.each(function(i2, elem) {
                    if (i2 % 2 === 0)
                      novelCover_1 = $2(elem).find("img").attr("src");
                    else {
                      novelName_1 = $2(elem).text().trim();
                      novelUrl_1 = $2(elem).find("a").attr("href");
                      if (!novelUrl_1)
                        return;
                      novel = {
                        name: novelName_1,
                  ...
                  txts.each(function(i2, elem) {
                    var novelName = $2(elem).text().trim();
                    var novelCover = $2(imgs_1[i2]).attr("src");
                    var novelUrl = $2(elem).find("a").attr("href");
                    if (novelUrl) {
                      novel = {
                        name: novelName,
                        cover: novelCover,
      ...
      ChireadsPlugin2.prototype.parseNovel = function(novelPath) {
        return __awaiter(this, void 0, void 0, function() {
          var novel, $2, infos, chapters, chapterList;
          var _this = this;
          return __generator(this, function(_a) {
            switch (_a.label) {
              case 0:
                novel = { path: novelPath, name: "Sans titre" };
                return [4, this.getCheerio(this.site + novelPath)];
              case 1:
                $2 = _a.sent();
                novel.name = $2(".inform-product-txt").first().text().trim() || $2(".inform-title").text().trim();
                novel.cover = $2(".inform-product img").attr("src") || $2(".inform-product-img img").attr("src") || defaultCover_1.defaultCover;
                novel.summary = $2(".inform-inform-txt").text().trim() || $2(".inform-intr-txt").text().trim();
                infos = $2("div.inform-product-txt > div.inform-intr-col").text().trim() || $2("div.inform-inform-data > h6").text().trim();
                ...
                chapters = [];
                chapterList = $2(".chapitre-table a");
                if (!chapterList.length) {
                  $2("div.inform-annexe-list").first().remove();
                  chapterList = $2(".inform-annexe-list").find("a");
                }
                chapterList.each(function(i, elem) {
                  var chapterName = $2(elem).text().trim();
                  var chapterUrl = $2(elem).attr("href");
      ...
      ChireadsPlugin2.prototype.parseChapter = function(chapterUrl) {
        return __awaiter(this, void 0, void 0, function() {
          var $2, chapterText;
          return __generator(this, function(_a) {
            switch (_a.label) {
              case 0:
                return [4, this.getCheerio(this.site + chapterUrl)];
              case 1:
                $2 = _a.sent();
                chapterText = $2("#content").html() || "";
                return [2, chapterText];
            }
          });
        });
      };
      ChireadsPlugin2.prototype.searchNovels = function(searchTerm, pageNo) {
        return __awaiter(this, void 0, void 0, function() {
          ...
          return __generator(this, function(_a) {
            switch (_a.label) {
              case 0:
                ...
                _a.label = 1;
              case 1:
                if (!!finised) return [3, 3];
                return [4, this.popularNovels(i, {
              ...
              case 3:
                novels = novels.filter(function(novel) {
                  return novel.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(searchTerm.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""));
                });
                return [2, novels];
            }
          });
